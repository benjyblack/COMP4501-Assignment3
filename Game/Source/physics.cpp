...
PxRigidStatic *PhysicsManager::physicsTerrain (Terrain *terrain) {
	...
	heightField = physicsSystem->createHeightField(description);

	//To position it at "physXPosition" in a scene, you need a static actor since it alone contains position information.
	Transformation fromTransformation;
	fromTransformation.preTranslateBy(physXPosition);
...
PxRigidStatic *PhysicsManager::physicsMesh (World *world) {
	...
	description.triangles.count = indices.size();
	description.points.stride = sizeof(PxVec3);
	description.triangles.stride = sizeof(PxU32)*3;
	description.points.data = &vertices[0];
	description.triangles.data = &indices[0];
...
PxRigidDynamic *PhysicsManager::physicsSphere (Point &position, Vector &velocity, float diameter, float mass) {
	...
	Transformation fromTransformation;
	fromTransformation.preTranslateBy(position);
...
PxRigidDynamic *PhysicsManager::physicsBox (Point &position, Vector &velocity, float width, float height, float depth, float mass) {
	...
	PxRigidDynamic* boxActor = physicsSystem->createRigidDynamic(asTransform(fromTransformation)); //You need to create it using the proper code...
...
bool PhysicsManager::sweptSphereHits (PxRigidDynamic *physicsSphere, Point &from, Point &to, double &intersectionT, Point &intersectionPoint) {
	...
	Transformation fromTransformation;
	fromTransformation.preTranslateBy(from);

	float toDistance;
	Vector direction = to-from;
	direction.normalize(toDistance);
	PxVec3 test = asVec3(direction);

	if (scene->sweepSingle(sphereGeometry, asTransform(fromTransformation), asVec3(direction), toDistance, PxSceneQueryFlag::eMESH_ANY | PxSceneQueryFlag::eDISTANCE, hit))
	{
...
bool PhysicsManager::sweptRayHits (Point &from, Point &to, double &intersectionT, Point &intersectionPoint) {
	...
	if (scene->raycastSingle(asVec3(from), asVec3(direction), toDistance, PxSceneQueryFlag::eMESH_ANY | PxSceneQueryFlag::eDISTANCE, hit))
	{
		float hitDistance = hit.distance;
